...
logger = logging.getLogger('shoppingcart.processors.paypal')
...
def start_payment_process(params):
   ...
   # for style in ('BODYCOLOR', 'HEADCOLOR', 'HEADLINECOLOR', 'MENUCOLOR', 'BODYFONTCOLOR', 'HEADFONTCOLOR', 'MENUFONTCOLOR', 'FONT'):
    ...
    response = requests.get(settings.CC_PROCESSOR['PayPal'].get('PROCESS_URL',''), params=data)
    logger.info('PayPal: order {0} redirected to PayPal gateway'.format(order_id))
    return {
...
def process_postpay_callback(params):
    ...
    logger.info('PayPal: order {0} verifying , DATA: {1}, SIGNATURE {2}'.format(order.pk, data['DATA'], data['SIGNATURE']))

    response = requests.get(settings.CC_PROCESSOR['PayPal'].get('VERIFY_URL'), params=data)
